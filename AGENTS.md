...
7. **Backend resolves members.** Memberships ordered by `recipientType` (TO/CC/BCC) then compact `priority`; admin add/edit reorders via `beforeRecipientId`/`afterRecipientId` and stores an optional `role`; INACTIVE recipients filtered out; response is `{ application, list, recipients: [{ name, email, type, role, priority }] }`.
...
- Parity rules (deviate only with reason): Nest-shaped errors `{ message, error, statusCode }`, camelCase + ISO timestamps + `_count`, pagination `{ total, page, limit }`, codes uppercased, `apiKey` only returned from generate/regenerate, `auth/me` never leaks the password hash, membership order = recipientType (TO/CC/BCC) then priority; memberships accept an optional `role` (max 255) and `beforeRecipientId`/`afterRecipientId` reordering (mutually exclusive; recomputes compact priorities), and the integration response carries `type`, `role`, `priority` per recipient.
